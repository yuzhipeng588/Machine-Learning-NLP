...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 15 22:44:36 2016
...
pinitial_test=pd.read_csv('pgpub_claims_fulltext.csv',delimiter=',',nrows=5000,skiprows=range(1,50000),encoding='utf-8')
pfinal_test=pd.read_csv('patent_claims_fulltext.csv',delimiter=',',nrows=5000,skiprows=range(1,50000),encoding='utf-8')
import numpy as np
...
emb_model.add(MaxPooling1D(pool_length=2)) # try CNN, for fun
emb_model.add(LSTM(128, dropout_W=0.2, dropout_U=0.2)) 


add_model = Sequential()
add_model.add(Dense(12, input_dim=1, init='normal', activation='relu',W_constraint = maxnorm(2)))
...
print('Train...')
model.fit([X_train_add,X_train], y_train, batch_size=batch_size, nb_epoch=2,
